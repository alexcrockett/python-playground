...
NOTE ON FRAGILITY (important for a reference repo):
    Yahoo's markup is generated by JavaScript tooling, so CSS class names such
    as `yf-1n64cj` are *hashed* and change on almost every deploy. Selecting on
    them will break often. Where possible we target stable attributes like
    `id` / `data-field`, which survive redesigns far longer. If you want robust
    data rather than a scraping exercise, prefer the `yfinance` package instead.
"""
...
# browser-like User-Agent. This is a single constant so it's easy to update.
