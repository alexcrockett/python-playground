# The script on this page converts temperatures between Celsius and Fahrenheit
...
def t_input(your_temperature):
    while True:  # The while loop is used for error validation
        ask_units = input(
            "What measurement do you want? Press 1 for Celsius, 2 for Fahrenheit: "
        )
        if ask_units == "1":
            return fahrenheit_to_celsius(
                your_temperature
            )  # We pass the user to the correct function with the argument
        elif ask_units == "2":
            ...
            print(
                "Invalid input. Please enter 1 for Celsius or 2 for Fahrenheit."
            )  # We try again
